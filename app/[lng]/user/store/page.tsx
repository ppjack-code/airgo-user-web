        ...
        result.data?.data?.map((item: any) => ({
          ...
          node_speed_limit:
            item.node_speed_limit === 0
              ? '无限制'
              : `${item.node_speed_limit / 1024 / 1024} Mbps`,
                ...
                <CardContent className="grid gap-1 p-0  text-sm md:gap-3 md:p-6">
                  ...
                  <ul className="grid gap-3 *:flex *:items-center *:justify-between">
                    <li>
                      ...
                      <span>
                        {
                          data.find((item: any) => item.id === order.goods_id)
                            .total_bandwidth
                        }
                      </span>
                    </li>
              ...
              <div className="flex flex-col justify-between text-sm">
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    const orderInfo =
                      await postCustomerOrderGetOrderInfoWaitPay(detail).then(
                        (res) => {
                    ...
                    switch (purchase.pay_type) {
                      case 'alipay':
                        window.location.href =
                          purchase.pay_info.alipay_info.qr_code;
                        break;
                      case 'epay':
                        const href = new URL(
                          purchase.pay_info.epay_info.epay_api_url,
                        );
                        Object.keys(
                          purchase.pay_info.epay_info.epay_pre_create_pay,
                        ).forEach((key) => {
